...
class Comic {
  ...
  /// List of episodes in this comic (loaded separately from subcollection)
  ...
  Comic({
    ...
    this.episodes = const [],  // Default to empty list since loaded separately
    required this.genre,
  ...
  Future<void> loadEpisodes() async {
    try {
      ...
      episodes = snapshot.docs.map((doc) {
        final data = doc.data();
        return Episode.fromMap({...data, 'id': doc.id});
      }).toList();
    } catch (e) {
      print('Error loading episodes for comic $id: $e');
      episodes = [];
    }
  }
}